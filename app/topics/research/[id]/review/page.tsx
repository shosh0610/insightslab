...
import { getResearchSession, type ResearchResult, type DiscoveredVideo } from '@/lib/api';
import { ArrowLeft, Sparkles, CheckCircle2, ExternalLink, FileText, Clock, AlertCircle } from 'lucide-react';
...
export default function ResearchReviewPage() {
  ...
  useEffect(() => {
    async function loadResearchSession() {
      try {
        ...
        // Pre-select videos that are already marked as selected
  ...
  const handleSynthesize = async () => {
    ...
    try {
      // Send selected video IDs to backend
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/research/${sessionId}/select-videos`,
        {
          method: 'POST',
      ...
      const synthesisResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/research/${sessionId}/synthesize`,
        {
          method: 'POST',
        }
      );
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <Button
                ...
                disabled={synthesizing || selectedVideos.length === 0}
                size="lg"
                className="gap-2"
              >
                {synthesizing ? (
